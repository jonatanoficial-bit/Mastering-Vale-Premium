...
export const ui = {
  topbar({ onNav, current, accentName }) {
    ...
    title.appendChild(
      el("span", {
        html: `Modo: <span style="color:var(--accent);font-weight:600">${accentName}</span>`,
      })
    );
  ...
  home({ genres, instruments, store, onBrowse, onFavorites }) {
    ...
    left.appendChild(
      el("div", { class: "hd" }, [
        el("div", {}, [
          ...
          el("p", {
            html: "Selecione gênero e instrumento, responda decisões reais e receba cadeias detalhadas + alvos de medição. Feito para mobile, pronto para mercado.",
          }),
        ]),
    ...
    bd.appendChild(
      el("div", { class: "row" }, [
        ...
        el("button", { class: "btn", onclick: onFavorites }, [
          el("span", { html: `Favoritos (${store.getFavorites().length})` }),
        ]),
    ...
    genres.slice(0, 8).forEach((g) => {
      ...
      it.appendChild(
        el("div", { class: "meta" }, [el("strong", { html: g.name }), el("span", { html: "Blueprints + Master modes" })])
      );
    ...
    instruments.slice(0, 6).forEach((i) => {
      const it = el("div", {
        class: "item",
        onclick: () => (location.hash = `#/blueprint?g=${genres[0]?.id || "pop"}&i=${i.id}&l=intermediate`),
      });
      it.appendChild(
        el("div", { class: "meta" }, [
  ...
  favorites({ items, onOpen, onClear }) {
    ...
    card.appendChild(
      el("div", { class: "hd" }, [
        el("div", {}, [el("h2", { html: "Favoritos" }), el("p", { html: "Seus blueprints salvos para acesso rápido (local)." })]),
  ...
  browse({ genres, instruments, selection, onSelectGenre, onSelectInstrument }) {
    ...
    gcard.appendChild(
      el("div", { class: "hd" }, [
        el("div", {}, [el("h2", { html: "1) Escolha o gênero" }), el("p", { html: "A identidade e recomendações adaptam conforme o estilo." })]),
  ...
  master({ genre, modes, onBack, onUnlock }) {
    ...
    modes.forEach((m) => {
      ...
      detail.appendChild(
        el("p", {
          html: `<span style="color:var(--muted)">Targets:</span> LUFS-I ${tg.lufs_i?.min}..${tg.lufs_i?.max} ${tg.lufs_i?.unit || ""} • TP ${tg.tp?.min}..${tg.tp?.max} ${tg.tp?.unit || ""}`,
        })
      );
  ...
  blueprint({
    ...
    for (const d of blueprint?.decisions || []) {
      if (d.type === "segmented") {
        ...
        box.appendChild(
          el("div", { class: "top" }, [
            ...
            el("span", {
              html: `Selecionado: <b style="color:var(--accent2)">${(d.options.find((o) => o.id === decisions[d.id]) || {}).label || ""}</b>`,
            }),
          ])
        );
      ...
      if (d.type === "slider") {
        const box = el("div", { class: "slider" });
        box.appendChild(el("div", { class: "top" }, [el("strong", { html: d.label }), el("span", { html: `${decisions[d.id]}` })]));
    ...
    right.appendChild(
      el("div", { class: "hd" }, [
        el("div", {}, [el("h2", { html: blueprint?.title || "Blueprint" }), el("p", { html: "Cadeia visual + alvos de medição (guia)." })]),
    ...
    (resolvedChain || []).forEach((s, idx) => {
      ...
      st.appendChild(
        el("div", { class: "cap" }, [el("strong", { html: `${idx + 1}. ${s.name}` }), el("span", { class: "t", html: (s.type || "stage").toUpperCase() })])
      );
    ...
    bd2.appendChild(
      el("p", {
        style: "margin:0;color:var(--muted);font-size:12px;line-height:1.45",
        html: "Observação: os meters são guias educacionais e profissionais (não medem áudio real). Ideal para orientar decisões, consistência e validação.",
      })
    );
  ...
  // ✅ AGORA O UPGRADE ESTÁ NO LUGAR CERTO (dentro do ui)
  upgrade() {
    const wrap = el("div", { class: "main" });
    ...
    [
      ...
      it.appendChild(
        el(
          "button",
          { class: "btn primary", onclick: () => alert("Checkout futuro") },
...
function info(title, desc) {
  const it = el("div", { class: "item" });
  it.appendChild(el("div", { class: "meta" }, [el("strong", { html: title }), el("span", { html: desc })]));
